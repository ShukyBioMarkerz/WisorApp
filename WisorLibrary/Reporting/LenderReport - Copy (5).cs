            ...
            try
            {
                ...
                if (CultureInformation.Name.Equals("he-IL"))
                {
                    ...
                    compositionSummaryRows += String.Format("<tr style='font-weight: bold;'>\r\n"
                                         + "<td>{0}</td>\r\n"
                                         + "<td style='position: relative;  border: 0px solid white; border-right: 1px solid black;'>{1}</td>\r\n"
                                         + "<td style='position: relative;  border: 0px solid white; border-left: 0px solid white;'>{2}</td>\r\n"
                                         + "<td style='position: relative;  border: 0px solid white; border-left: 0px solid white;'>{3}</td>\r\n"
                                         + "<td style='position: relative;  border: 0px solid white; border-left: 1px solid black;'>{4}</td>\r\n"
                                         + "<td>{5}</td>\r\n"
                                         + "<td>{6}</td>\r\n"
                                         + "<td>{7}</td>\r\n"
                                         + "<td>{8}</td>\r\n"
                ...
                else // including en-US
                {
                    ...
                    page = String.Format(LenderPages[1]
                                          ...
                                          , "0" //MiscUtilities.ConvertDate(reportData.DateTaken.ToShortDateString())    // Date taken
                                          , "0" //CurrencySymbol + reportData.OriginalLoanAmount.ToString("N0")              // Amount taken
                                          , reportData.ProductName                                      // Product                                                                        
                                          , reportData.OriginalTime                                     // Term                                                              
                                          , "0" //Math.Round(reportData.OriginalRate * 100, 3) + "%"                          // Rate
                                          , "0" //CurrencySymbol + reportData.FirstMonthlyPMT.ToString("N0")                 // First monthly payment
                                          , "0" //CurrencySymbol + reportData.YearlyIncome.ToString("N0")                    // Total income
                                          , "0" //Math.Round(reportData.EstimateFutureProfitPercantage * 100, 3) + "%"             // Debt-to-incme
                                          , "0" //CurrencySymbol + reportData.PayUntilToday.ToString("N0")                   // Paid until today
                                          , "0" //CurrencySymbol + reportData.PayFuture.ToString("N0")                       // Left to pay
                                          , "0" //CurrencySymbol + reportData.EstimateFuturePay.ToString("N0")               // Estimated future payment

                                          , reportData.OriginalMargin + "%"                             // Product margin                                                              
                                          , "0" //Math.Round(reportData.EstimateProfitPercantageSoFar * 100, 3) + "%"              // Estimated % profits so far
                                          , "0" //CurrencySymbol + reportData.EstimateProfitSoFar.ToString("N0")             // Estimated profit so far
                                          , "0" //Math.Round(reportData.EstimateTotalProfitPercantage * 100, 3) + "%"              // Estimated total % profit
                                          , "0" //CurrencySymbol + reportData.EstimateTotalProfit.ToString("N0")             // Estimated total profit
                                          , "0" //Math.Round(reportData.EstimateFutureProfitPercantage * 100, 3) + "%"             // Estimated future % profit
                                          , CurrencySymbol + reportData.EstimateFutureProfit.ToString("N0")            // Estimated future profit
