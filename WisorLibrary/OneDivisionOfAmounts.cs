...
namespace WisorLib
{
    class OneDivisionOfAmounts
    {
        ...
        public OneDivisionOfAmounts(double amtOpt1, double amtOpt2, double amtOpt3)
        {
            ...
            pmtLimits = new PmtLimits(amtOptX, amtOptY, amtOptZ);
            fixedOptZ0 = FindZ0(CalculationParameters.optTypes, pmtLimits, 
                                    CalculationParameters.optTypes.optionTypes[(int)Options.options.OPTZ].minTime);
            if (fixedOptZ0 != null)
            {
                while ((fixedOptZ0 != null) && 
                            (fixedOptZ0.optTime <= CalculationParameters.optTypes.optionTypes[(int)Options.options.OPTZ].maxTime))
                {

                    plane = new SinglePlane(pmtLimits, fixedOptZ0);
                    
                    savedCompositionsForFile = new SavedCompositions(fixedOptZ0, plane.savedMatches);
                    
                    if (fixedOptZ0.optTime == CalculationParameters.optTypes.optionTypes[(int)Options.options.OPTZ].maxTime)
                    {
                        fixedOptZ0 = null;
                    }
                    else
                    {
                        fixedOptZ0 = FindZ0(CalculationParameters.optTypes, pmtLimits,
                                        (fixedOptZ0.optTime + CalculationParameters.optTypes.optionTypes[(int)Options.options.OPTZ].jump));
                    }
                }
            }
            else
        ...
        private Option FindZ0(OptionTypes optTypes, PmtLimits pmtLimits, uint timeZ)
        {
            ...
            if ((ttlpmtmaxT > CalculationParameters.monthlyPmtWanted + CalculationConstants.smallDev) ||
                    (ttlpmtminT < CalculationParameters.monthlyPmtWanted - CalculationConstants.largeDev))
            {
                if (ttlpmtmaxT > CalculationParameters.monthlyPmtWanted + CalculationConstants.smallDev)
                {
                    if (printOrNo == true)
                    {
                        Console.WriteLine("\nTarget monthly payment " + CalculationParameters.monthlyPmtWanted
                                            + " smaller than minimum PMT " + ttlpmtmaxT + "\nNo possible matches for this division of money\n");
                    }
                    optionZ0 = null;
                }
                else if (ttlpmtminT < CalculationParameters.monthlyPmtWanted - CalculationConstants.largeDev)
                {
                    if (printOrNo == true)
                    {
                        Console.WriteLine("\nTarget monthly payment " + CalculationParameters.monthlyPmtWanted
            ...
            {
                optionZ0 = new Option(CalculationParameters.optTypes.optionTypes[(int)Options.options.OPTZ].typeId,
                ...
                if ((ttlPMTmaxTime > CalculationParameters.monthlyPmtWanted + CalculationConstants.smallDev) ||
                                    (ttlPMTminTime < CalculationParameters.monthlyPmtWanted - CalculationConstants.largeDev))
                {
                    if (ttlPMTmaxTime > CalculationParameters.monthlyPmtWanted + CalculationConstants.smallDev)
                    {
                        if (printOrNo == true)
                        {
                            Console.WriteLine(CalculationParameters.monthlyPmtWanted.ToString() + " < " + ttlPMTmaxTime.ToString()
                        ...
                        else
                        {
                            ...
                            return FindZ0(optTypes, pmtLimits, (timeZ + optTypes.optionTypes[(int)Options.options.OPTZ].jump));
                        }
                    }
                    else if (ttlPMTminTime < CalculationParameters.monthlyPmtWanted - CalculationConstants.largeDev)
                    {
                        if (printOrNo == true)
                        {
                            Console.WriteLine(ttlPMTminTime.ToString() + " < " + CalculationParameters.monthlyPmtWanted.ToString()
